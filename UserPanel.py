...
class MyApp(object):
    def __init__(self, parent):
        ...
        self.btn_admin = tk.Button(self.frame, text="Admin Login", highlightthickness=0, bg="white", command=self.AdminEntryPage)
        ...
        self.ent.bind('<Return>', self.fill_table_rows)
    ...
    def fill_table_rows(self, event=None):
        list = bdb.list_books()
        ...
        for i in list:
            ...
            if self.sname.get() == "" or self.sname.get() == " ":
                self.tree.insert("", 'end', text=index, values=(i[1], i[2], i[3], dtype.get_book_by_id[i[4]], dtype.get_status_by_id[i[5]]))
                continue
            for ch in i:
    ...
    def fill_table_columns(self):
        ...
        self.tree.heading('three', text="Published Date", anchor=tk.W)
    ...
    def openAdminPanel(self, arg1, arg2=None):
        root.withdraw()
        subFrame = Admin()
    ...
    def verifyDatabases(self):
        ...
        if not udb.check_database():
            udb.create_database()
            self.fillUsers()
        if not adb.check_database():
            adb.create_database()
            self.fillAdmins()

    def fillBooks(self):
        bdb.insert_book_silence("uygarligin ayak izleri", "celil sadik", 1940, 1)
        bdb.insert_book_silence("türklerle beraber", "emre sarıl", 2018, 2)
        bdb.insert_book_silence("sevgili wanda", "gephart hauptman", 1999, 1)
        bdb.insert_book_silence("kuyucakli yusuf", "sabahattin ali", 1999, 3)
        bdb.insert_book_silence("beyaz geceler", "fyodor dostoyevski", 1999, 1)
        bdb.insert_book_silence("palto", "nikolay gogol", 1999, 1)
        bdb.insert_book_silence("dokuzuncu harbiye kogusu", "peyafi safa", 1999, 4)
        bdb.insert_book_silence("vurun kahpeye", "halide edip adivar", 1999, 1)
        bdb.insert_book_silence("yilanlarin ocu", "fakir baykurt", 1999, 6)
        bdb.insert_book_silence("gulyabani", "huseyin rahmi gunpinar", 1999, 1)
        bdb.insert_book_silence("esir sehrin insanlari", "kemal tahir", 1999, 7)
        bdb.insert_book_silence("maviye boyanmis sular", "cemil kavukcu", 1999, 8)
        bdb.insert_book_silence("insanciklar", "fyodor dostoyevski", 1999, 9)
        bdb.insert_book_silence("mai ve siyah", "halit ziya usakligil", 1999, 10)
        bdb.insert_book_silence("istanbul hatirasi", "ahmet umit", 1999, 12)
        bdb.insert_book_silence("devlet ana", "kemal tahir", 1999, 3)
        bdb.insert_book_silence("fosforlu cevriye", "suat dervis", 1999, 5)
        bdb.insert_book_silence("fatih harbiye", "peyami safa", 1999, 7)

    def fillUsers(self):
        udb.insert_user_silence("can", "yilmaz", 5073022302, "canyil97@hotmail.com", 2020)
        udb.insert_user_silence("ozlem", "acar", 5453022301, "safsafsa@hotmail.com", 2020)
        udb.insert_user_silence("ozde", "acarkan", 5073024533, "asdsadsa@gmail.com", 2019)
        udb.insert_user_silence("atahan", "adanir", 5073524533, "dfsf@gmail.com", 2018)
        udb.insert_user_silence("mehmet", "adiguzel", 5073064533, "asdsafsdag@gmail.com", 2020)
        udb.insert_user_silence("bestami", "agnar", 5074024533, "xcvxcvxc@gmail.com", 2020)
        udb.insert_user_silence("aslan", "ahmet", 5073124533, "nghnhg@gmail.com", 2020)
        udb.insert_user_silence("sennur", "agca", 5043024533, "asdsadsa@htyh.com", 2020)
        udb.insert_user_silence("tutkum", "akarcali", 5173024533, "yhthty@gmail.com", 2020)
        udb.insert_user_silence("nuhaydar", "akay", 5873024533, "utyuty@gmail.com", 2020)
        udb.insert_user_silence("servet", "akca", 5073624533, "dfgdfhdf@gmail.com", 2020)

    def fillAdmins(self):
        adb.insert_admin("admin", "admin")
